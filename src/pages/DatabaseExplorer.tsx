           ...
           <div className="space-y-4">
              ...
              <div className="space-y-1">
                {tables.map(t => (
                  <button 
                    key={t}
                    onClick={() => { setActiveTable(t); setQuery(`SELECT * FROM ${t} LIMIT 10;`); }}
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
           <div className="glass-card flex flex-col p-0 overflow-hidden bg-black/40">
              <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
                 ...
                 <div className="flex gap-2">
                    <button className="p-2 glass rounded-lg text-white/20 hover:text-white"><Save size={14} /></button>
                    <button className="p-2 glass rounded-lg text-white/20 hover:text-white"><RefreshCcw size={14} /></button>
                    <button className="btn-primary py-1.5 px-4 text-[10px] uppercase font-bold tracking-widest rounded-lg flex items-center gap-2">
           ...
           <div className="glass-card flex-1 p-0 overflow-hidden relative">
              <div className="p-4 border-b border-white/10 flex justify-between items-center">
                 ...
                 <span className="text-[10px] font-bold text-accent px-2 py-0.5 bg-accent/10 rounded-full tracking-tighter">Execution: 0.04ms</span>
              </div>
                 ...
                 <table className="w-full text-left border-collapse">
                    <thead className="bg-white/5 text-[10px] font-bold uppercase tracking-widest text-white/40 sticky top-0 backdrop-blur-md">
                        {activeTable === 'action_logs' ? (
                          ...
                          <tr>
                            <th className="px-6 py-3 border-b border-white/5">id</th>
                            <th className="px-6 py-3 border-b border-white/5">username</th>
                            <th className="px-6 py-3 border-b border-white/5">role</th>
                            <th className="px-6 py-3 border-b border-white/5">department</th>
                          </tr>
                        )}
                     </thead>
                        ...
                        {activeTable === 'action_logs' ? (
                          logs.length > 0 ? logs.map((log: any) => (
                            <tr key={log.id} className="hover:bg-white/5">
                              <td className="px-6 py-4 text-white/40">{log.id.substring(0, 8)}...</td>
                          ...
                          [1,2,3,4,5].map(i => (
                            <tr key={i} className="hover:bg-white/5">
                              <td className="px-6 py-4 text-white/40">{i}</td>
                              <td className="px-6 py-4 font-bold text-white/80">system_node_0{i}</td>
                              <td className="px-6 py-4">
                                <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 scale-90 inline-block font-sans font-bold">USER</span>
                              </td>
                              <td className="px-6 py-4 text-white/20 italic tracking-widest">ENCRYPTED</td>
                            </tr>
                          ))
                        )}
                     </tbody>
